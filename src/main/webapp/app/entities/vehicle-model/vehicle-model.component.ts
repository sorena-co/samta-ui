...
export class VehicleModelComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.vehicleModel.havePrimitiveMeasure = null;
        ...
        for (const key of x) {
            ...
            if (value.length > 1) {
                if (value[0] === 'havePrimitiveMeasure') {
                    this.vehicleModel[value[0]] = Boolean(value[1]);
                }
                else {
                    this.vehicleModel[value[0]] = value[1];
                }
            }
        }
    }
    ...
    search() {
        ...
        if (this.vehicleModel.title) {
            this.currentSearch += 'title$' + this.vehicleModel.title + '&';
        }
        ...
        if (this.vehicleModel.havePrimitiveMeasure) {
            this.currentSearch += 'havePrimitiveMeasure;' + this.vehicleModel.havePrimitiveMeasure + '&';
        }
    ...
    registerChangeInVehicleModels() {
        this.eventSubscriber = this.eventManager.subscribe('vehicleModelListModification', (response) => this.loadAll());
    }
