...
currentAccount: any;
    ...
    loadAll() {
        this.loanPaymentService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<LoanPayment[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/loan-payment'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: LoanPayment) {
        return item.id;
    }
    registerChangeInLoanPayments() {
        this.eventSubscriber = this.eventManager.subscribe('loanPaymentListModification', (response) => this.loadAll());
    }
