...
export class BillService {
    ...
    private resourcCustomerDeactiveeUrl = SERVER_API_URL + 'niopdcbase/api/customer-deactive-rules';
    ...
    checkActive(bill: Bill): Observable<BooleanResponseType> {
        const copy = this.convert(bill);
        return this.http.post<boolean>(`${this.resourcCustomerDeactiveeUrl}/active`, copy, {observe: 'response'})
    ...
    queryForReport(id: any): Observable<EntityResponseType> {
        return this.http.get<Bill>(`${this.resourceUrl}/report/${id}`, {observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }
