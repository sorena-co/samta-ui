...
export class BoundaryDiscountService {
    ...
    query(locationId, countryId, vehicleModelId, productId, req?: any): Observable<HttpResponse<BoundaryDiscount[]>> {
        let options = createRequestOption(req);
        if (locationId) {
            options = options.set("locationId", locationId);
        }
        if (countryId) {
            options = options.set("countryId", countryId);
        }
        if (vehicleModelId) {
            options = options.set("vehicleModelId", vehicleModelId);
        }
        if (productId) {
            options = options.set("productId", productId);
        }
