...
export class OrderComponent implements OnInit, OnDestroy {
    ...
    search() {
        ...
        if (this.order.lastModifiedDate) {
            this.currentSearch += 'lastModifiedDate☫' + this.order.lastModifiedDate.toISOString() + '&';
        }
        if (this.order.price) {
    ...
    setBreadCrumb() {
        ...
        if (this.mode === 'airplane') {
            type = 'order_airplane';
        } else if (this.mode === 'exchange') {
            type = 'exchange';
        } else if (this.mode === 'export') {
            type = 'export';
        }

        this.translateService.get('global.menu.sell.' + type).subscribe((title) => {
    ...
    ngOnInit() {
        ...
        this.route.params.subscribe((params) => {
            ...
            this._hotkeysService.add(new Hotkey(['ins', 'enter'], (event: KeyboardEvent, k: string): boolean => {
                if (k === 'ins') {
                    this.router.navigate(['order/order-new/' + this.mode]);
                    return false;
                }
            }));
        });
