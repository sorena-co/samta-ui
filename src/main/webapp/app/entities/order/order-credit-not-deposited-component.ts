...
import {BankTransactionRef} from '../bank-transaction-ref/bank-transaction-ref.model';
import {SpentType} from '../payment-bill/payment-bill.model';
...
export class OrderCreditNotDepositedComponent implements OnInit, OnDestroy {
    ...
    setBreadCrumb() {
        ...
        this.translateService.get('samtagatewayApp.order.home.title').subscribe((title) => {
    ...
    payment() {
        ...
        bt.redirectUrl = '/order/credit-not-deposited';
        bt.type = SpentType.CREDIT;
        bt.bankTransactionRefs = [];
        this.orderPayments.forEach((order) => {
            const btr = new BankTransactionRef();
            btr.amount = order.price;
            btr.personId = order.personId;
            btr.orderId = order.id;
            btr.customerId = order.customerId;
