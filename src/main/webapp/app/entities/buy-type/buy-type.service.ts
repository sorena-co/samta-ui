import {Injectable} from '@angular/core';
import {HttpClient, HttpParams, HttpResponse} from '@angular/common/http';
import {Observable} from 'rxjs/Observable';
import {SERVER_API_URL} from '../../app.constants';

import {BuyGroup, BuyType} from './buy-type.model';
import {createRequestOption} from '../../shared';
import {ContractType} from "../sell-contract";
import {TypeOfFuelReceipt} from "../order";

export type EntityResponseType = HttpResponse<BuyType>;

@Injectable()
export class BuyTypeService {

    private resourceUrl = SERVER_API_URL + 'niopdcbase/api/buy-types';
    private resourceCustomerUrl = SERVER_API_URL + '/niopdcbase/api/customers';

    constructor(private http: HttpClient) {
    }

    create(buyType: BuyType): Observable<EntityResponseType> {
        const copy = this.convert(buyType);
        return this.http.post<BuyType>(this.resourceUrl, copy, {observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    update(buyType: BuyType): Observable<EntityResponseType> {
        const copy = this.convert(buyType);
        return this.http.put<BuyType>(this.resourceUrl, copy, {observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<BuyType>(`${this.resourceUrl}/${id}`, {observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    query(req?: any): Observable<HttpResponse<BuyType[]>> {
        const options = createRequestOption(req);
        return this.http.get<BuyType[]>(this.resourceUrl, {params: options, observe: 'response'})
            .map((res: HttpResponse<BuyType[]>) => this.convertArrayResponse(res));
    }

    queryForCustomerCredit(customerId: number): Observable<HttpResponse<BuyType[]>> {
        let params;
        if (customerId) {
            params = new HttpParams().set("customerId", customerId.toString());
        }
        return this.http.get<BuyType[]>(this.resourceUrl + '/customer-credit', {params, observe: 'response'})
            .map((res: HttpResponse<BuyType[]>) => this.convertArrayResponse(res));
    }

    queryForSellContractProduct(customerId: number): Observable<HttpResponse<BuyType[]>> {
        let params;
        if (customerId) {
            params = new HttpParams().set("customerId", customerId.toString());
        }
        return this.http.get<BuyType[]>(this.resourceUrl + '/sell-contract-product', {params, observe: 'response'})
            .map((res: HttpResponse<BuyType[]>) => this.convertArrayResponse(res));
    }

    queryByDepotAndCustomerAnd(currencyRateGroupId: number, currencyId: number, depotId: number, customerId: number, personId: number) {
        return this.http.get(`${this.resourceCustomerUrl}/${currencyRateGroupId}/${currencyId}/${depotId}/${customerId}/${personId}/sell-contracts/sell-contract-products`,
            {observe: 'response'})
            .map((res: HttpResponse<BuyType[]>) => this.convertArrayResponse(res));
    }

    findBuyGroupByDepotAndCustomerAnd(currencyRateGroupId: number, currencyId: number, depotId: number, customerId: number, personId: number, sellContractId: number, contractType: ContractType) {
        let params;
        if (customerId == null) {
            if (currencyRateGroupId == null)
                params = new HttpParams()
                    .set('contractType', contractType.toString())
                    .set('sellContractId', sellContractId.toString());
            else
                params = new HttpParams()
                    .set('contractType', contractType.toString())
                    .set('currencyRateGroupId', currencyRateGroupId.toString())
                    .set('sellContractId', sellContractId.toString());


        } else {
            if (currencyRateGroupId == null)
                params = new HttpParams()
                    .set('customerId', customerId.toString())
                    .set('sellContractId', sellContractId.toString())
                    .set('contractType', contractType.toString());
            else
                params = new HttpParams()
                    .set('customerId', customerId.toString())
                    .set('sellContractId', sellContractId.toString())
                    .set('currencyRateGroupId', currencyRateGroupId.toString())
                    .set('contractType', contractType.toString());
        }

        return this.http.get(`${this.resourceCustomerUrl}/${currencyId}/${depotId}/${personId}/sell-contracts/sell-contract-products/buyGroup`,
            {params, observe: 'response'})
            .map((res: HttpResponse<BuyGroup[]>) => this.convertBuyGroupArrayResponse(res));
    }

    findTypeOfFuelReceiptByDepotAndCustomer(currencyRateGroupId: number, currencyId: number, depotId: number, customerId: number, personId: number, sellContractId: number, contractType: ContractType, buyType: BuyType) {
        let params;
        if (currencyRateGroupId != null)
            params = new HttpParams()
                .set('customerId', customerId.toString())
                .set('sellContractId', sellContractId.toString())
                .set('contractType', contractType.toString())
                .set('currencyRateGroupId', currencyRateGroupId.toString())
                .set('buyGroup', buyType.toString());
        else
            params = new HttpParams()
                .set('customerId', customerId.toString())
                .set('sellContractId', sellContractId.toString())
                .set('contractType', contractType.toString())
                // .set('currencyRateGroupId', currencyRateGroupId.toString())
                .set('buyGroup', buyType.toString());

        return this.http.get(`${this.resourceCustomerUrl}/${currencyId}/${depotId}/${personId}/sell-contracts/sell-contract-products/typeOfFuelReceipt`,
            {params, observe: 'response'})
            .map((res: HttpResponse<TypeOfFuelReceipt[]>) => this.convertTypeOfFuelReceiptArrayResponse(res));
    }

    delete(id: number): Observable<HttpResponse<any>> {
        return this.http.delete<any>(`${this.resourceUrl}/${id}`, {observe: 'response'});
    }

    private convertResponse(res: EntityResponseType): EntityResponseType {
        const body: BuyType = this.convertItemFromServer(res.body);
        return res.clone({body});
    }

    private convertArrayResponse(res: HttpResponse<BuyType[]>): HttpResponse<BuyType[]> {
        const jsonResponse: BuyType[] = res.body;
        const body: BuyType[] = [];
        for (let i = 0; i < jsonResponse.length; i++) {
            body.push(this.convertItemFromServer(jsonResponse[i]));
        }
        return res.clone({body});
    }

    private convertBuyGroupArrayResponse(res: HttpResponse<BuyGroup[]>): HttpResponse<BuyGroup[]> {
        const jsonResponse: BuyGroup[] = res.body;
        const body: BuyGroup[] = [];
        for (let i = 0; i < jsonResponse.length; i++) {
            body.push(this.convertBuyGroupItemFromServer(jsonResponse[i]));
        }
        return res.clone({body});
    }

    private convertTypeOfFuelReceiptArrayResponse(res: HttpResponse<TypeOfFuelReceipt[]>): HttpResponse<TypeOfFuelReceipt[]> {
        const jsonResponse: TypeOfFuelReceipt[] = res.body;
        const body: TypeOfFuelReceipt[] = [];
        for (let i = 0; i < jsonResponse.length; i++) {
            body.push(this.convertTypeOfFuelReceiptItemFromServer(jsonResponse[i]));
        }
        return res.clone({body});
    }

    /**
     * Convert a returned JSON object to BuyType.
     */
    private convertItemFromServer(buyType: BuyType): BuyType {
        const copy: BuyType = Object.assign({}, buyType);
        return copy;
    }

    private convertBuyGroupItemFromServer(buyGroup: BuyGroup): BuyGroup {
        return buyGroup;
    }

    private convertTypeOfFuelReceiptItemFromServer(typeOfFuelReceipt: TypeOfFuelReceipt): TypeOfFuelReceipt {
        return typeOfFuelReceipt;
    }

    /**
     * Convert a BuyType to a JSON which can be sent to the server.
     */
    private convert(buyType: BuyType): BuyType {
        const copy: BuyType = Object.assign({}, buyType);
        return copy;
    }
}
