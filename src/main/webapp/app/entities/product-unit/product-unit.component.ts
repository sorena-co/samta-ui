...
export class ProductUnitComponent implements OnInit, OnDestroy {
    ...
    constructor(private productUnitService: ProductUnitService,
        ...
        for (const key of x) {
            ...
            if (value.length > 1) {
                this.productUnit[value[0]] = value[1];
            }
        }
    }
    ...
    loadAll() {
        this.productUnitService.query({
            ...
            (res: HttpResponse<ProductUnit[]>) => this.onSuccess(res.body, res.headers),
    ...
    search() {
        this.page = 0;
        this.currentSearch = '';
        if (this.productUnit.title) {
            this.currentSearch += 'title$' + this.productUnit.title + '&';
        }
        if (this.productUnit.code) {
            this.currentSearch += 'code$' + this.productUnit.code + '&';
        }
    ...
    ngOnInit() {
        ...
        this.registerChangeInProductUnits();
    ...
    trackId(index: number, item: ProductUnit) {
        return item.id;
    }

    registerChangeInProductUnits() {
        this.eventSubscriber = this.eventManager.subscribe('productUnitListModification', (response) => this.loadAll());
    }
