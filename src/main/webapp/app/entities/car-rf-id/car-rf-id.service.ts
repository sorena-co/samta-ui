...
export class CarRfIdService {
    ...
    countByCustomerId(customerId: number): Observable<NumberResponseType> {
        return this.http.get<number>(`${this.resourceUrl}/count-rf-id/${customerId}`, {observe: 'response'})
            .map((res: NumberResponseType) => res);
    }

    query(customerId: any, req?: any): Observable<HttpResponse<CarRfId[]>> {
        const options = createRequestOption(req);
        return this.http.get<CarRfId[]>(this.resourceCustomerUrl + '/' + customerId + '/car-rf-ids', {
            params: options,
            observe: 'response'
        })
