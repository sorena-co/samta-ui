...
export class RefuelCenterService {
    ...
    constructor(private http: HttpClient, private locationService: LocationService, private regionService: RegionService) {
    }
    ...
    /*queryByRefuelCenterIds(refuelCenterIds: number[]|any): Observable<HttpResponse<RefuelCenter[]>> {
        const options = new HttpParams().set('ids', refuelCenterIds.toString());
        return this.http.post<RefuelCenter[]>(this.resourceUrl, {params: options, observe: 'response'})
            .map((res: HttpResponse<RefuelCenter[]>) => this.convertArrayResponse(res));
    }*/
    /*findAirportByRefuelCenterIds(refuelCenterIds: number): Observable<HttpResponse<RefuelCenter[]>> {
        const options = new HttpParams().set('ids', refuelCenterIds);
        return this.http.post<RefuelCenter[]>(`${this.resourceUrl}/refuel-centers-by-ids`, {params: options, observe: 'response'}, {params: options, observe: 'response'})
            .map((res: HttpResponse<RefuelCenter[]>) => this.convertArrayResponse(res));
    }*/

   /* queryBySubLocationsAndLevel(ids): Observable<HttpResponse<RefuelCenter[]>> {
        const options = new HttpParams().set('ids', refuelCenterIds.toString());
        return this.http.post<RefuelCenter[]>(this.resourceUrl + '/refuel-centers-by-ids' ,
