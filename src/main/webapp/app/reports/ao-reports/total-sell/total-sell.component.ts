...
export class TotalSellComponent implements OnInit, OnDestroy {
    ...
    reverse: any;
    breadcrumbItems: any[];
    req: TotalSellRequest = new TotalSellRequest({});
    aggregates: any[] = [{field: 'amount', aggregate: 'sum'}];
    state: State = {take: 10};
    gridData: any = process(this.totalSell, this.state);
    ...
    constructor(
        private totalSellService: TotalSellService,
    ...
    loadAll() {
        this.totalSellService.query(this.req).subscribe(
            (res: HttpResponse<TotalSell[]>) => this.onSuccess(res.body),
    ...
    setBreadCrumb() {
        ...
        this.translateService.get('niopdcgatewayApp.totalSell.home.title').subscribe((title) => {
    ...
    private onSuccess(data) {
        ...
        this.gridData = process(this.totalSell, this.state);
    }
