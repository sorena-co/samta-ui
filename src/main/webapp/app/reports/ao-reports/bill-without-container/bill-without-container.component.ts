...
@Component({
    selector: 'jhi-bill-without-container',
    templateUrl: './bill-without-container.component.html'
})
export class BillWithoutContainerComponent implements OnInit, OnDestroy {

    currentAccount: any;
    billWithoutContainer: BillWithoutContainer[] = [];
    error: any;
    success: any;
    ...
    reverse: any;
    breadcrumbItems: any[];
    req: BillWithoutContainerRequest = new BillWithoutContainerRequest({});
    aggregates: any[] = [{field: 'amount', aggregate: 'sum'}];
    state: State = {take: 10};
    gridData: any = process(this.billWithoutContainer, this.state);
    ...
    constructor(
        private billWithoutContainerService: BillWithoutContainerService,
    ...
    loadAll() {
        this.billWithoutContainerService.query(this.req).subscribe(
            (res: HttpResponse<BillWithoutContainer[]>) => this.onSuccess(res.body),
    ...
    setBreadCrumb() {
        ...
        this.translateService.get('niopdcgatewayApp.billWithoutContainer.home.title').subscribe((title) => {
