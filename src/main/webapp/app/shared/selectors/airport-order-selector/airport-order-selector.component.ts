...
import {Airport, AirportService} from '../../../entities/airport';
...
export class AirportOrderSelectorComponent implements OnInit {
            ...
            if (this.multiSelect && val instanceof Array) {
                ...
                val.forEach((value) => {
                        if (typeof value === 'number' && this.selects.find((value_f) => value_f.id === value) == null) {
                            this.airportService.find(value).subscribe(
                ...
                this.airportService.find(val).subscribe(
                    (res: HttpResponse<Airport>) => {
