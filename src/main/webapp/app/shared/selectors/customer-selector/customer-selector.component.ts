...
@Component({
    selector: 'jhi-customer-selector',
...
export class CustomerSelectorComponent implements OnInit, OnChanges {
    ...
    @Input() customerHaveSellContract = true;
    @Input() creator = false;
    ...
    loadAll() {
        ...
        this.customerService.query({
            ...
            sort: this.sort(),
            haveSellContract: this.customerHaveSellContract,
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes.contractType && !changes.contractType.isFirstChange()) {
            this.loadAll();
        }
    }
            ...
            if (this.multiSelect && val instanceof Array) {
                ...
                val.forEach((value) => {
                        if (typeof value === 'number' && this.selects.find((value_f) => value_f.id === value) == null) {
                            this.customerService.find(value).subscribe(
                ...
                this.customerService.find(val).subscribe(
                    (res: HttpResponse<Customer>) => {
